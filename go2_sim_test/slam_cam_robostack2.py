...
class Go2SlamPublisher(Node):
    ...
    def update_robot(self) -> None:
        ...
        if self.articulation:
            ...
            # 베이스 포즈 업데이트 (Odometry 따름 - slam_cam.py 방식)
            # fix_base=True 인 상태에서 강제로 위치를 지정하여 넘어짐 방지
    ...
    def publish_data(self):
        ...
        # 2. 오도메트리 및 TF 발행 (World 객체에서 로봇 상태 조회)
        if self.robot_prim:
            # 로봇의 현재 위치 및 방향 가져오기 (Articulated System)
            # Articulation이 아닌 Prim 자체의 World Pose를 가져오는 것이 안전할 수 있음
            # 하지만 World.scene.get_object("go2")로 Articulation 객체를 가져와 get_world_pose()를 쓰는 것이 정석
            # 여기서는 Prim API 사용
            try:
                from omni.isaac.core.utils.prims import get_prim_at_path

                # World 객체에서 관리되는 Articulation을 사용하는 것이 좋지만,
                # 여기서는 독립성을 위해 Prim API 사용
                # transform = UsdGeom.Xformable(self.robot_prim).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
                # translation = transform.ExtractTranslation()
                # rotation = transform.ExtractRotationQuat()
                # -> get_world_pose는 Articulation view가 필요함.
                # 여기서는 간단히 Xformable API 사용
                pass
            except:
                pass

            # TODO: 정확한 오도메트리를 위해 World에서 Articulation 객체 참조를 받아오는 구조로 개선 필요
            # 현재는 카메라 시각화가 주 목적이므로 이 부분은 스킵하거나 추후 연결
